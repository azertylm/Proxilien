    ...
    <div className="space-y-3 w-full">
      ...
      {inProgressRequests.map((req) => {
        ...
        return (
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 pb-3 border-b border-amber-200/60 dark:border-slate-800">
              <div className="flex items-center gap-2.5 min-w-0">
                <span className="text-2xl sm:text-3xl p-1.5 rounded-xl bg-orange-100 dark:bg-orange-950/80 flex-shrink-0">
                  {isArrived ? '🔔' : isEnRoute ? '🚗' : '🌟'}
                </span>
                <div className="min-w-0">
                  ...
                  <h2 className={`font-black tracking-tight mt-0.5 truncate ${titleScaleClass} ${
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-bold text-xs sm:text-sm bg-amber-100 hover:bg-amber-200 text-amber-950 border border-amber-300 transition cursor-pointer flex-shrink-0 self-end sm:self-auto"
              ...
              <div className="flex items-center justify-between md:justify-end gap-3 flex-wrap">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <span className="text-xs font-bold text-emerald-700 dark:text-emerald-400 flex items-center gap-1">
                      ...
                      <span>{req.helperRole || 'Jeune voisin certifié'}</span>
                    </span>
                  </div>
                </div>
                ...
                {req.helperPhone && (
                  <a
                    ...
                    className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-3.5 py-2 rounded-xl text-xs sm:text-sm flex items-center gap-1.5 shadow-xs transition"
            ...
            {req.arrivalNote && (
              <div className="border-l-3 border-orange-500 bg-orange-50/70 dark:bg-orange-950/30 px-3 py-1.5 rounded-r-lg my-2 flex items-start gap-2">
            ...
            <div className="pt-2 border-t border-amber-200/50 dark:border-slate-800 flex items-center justify-end">
              <button
                type="button"
                id={`btn-confirm-resolved-${req.id}`}
                onClick={() => onConfirmResolved(req.id)}
                className="w-full sm:w-auto bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-700 hover:to-amber-700 text-white font-black px-5 py-2.5 rounded-xl text-sm sm:text-base shadow-sm flex items-center justify-center gap-2 cursor-pointer transition active:scale-98"
              >
                <CheckCircle2 className="w-5 h-5 stroke-[2.5]" />
                <span>C'est fait, merci ! (Valider le coup de main)</span>
              </button>
            </div>
          </div>
        );
      ...
      {pendingRequests.map((req) => (
        <div
          ...
          className={`rounded-2xl p-3.5 sm:p-4 border transition flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 ${
            isDark ? 'bg-slate-900 border-slate-700 text-white' : 'bg-white border-amber-200 text-slate-900 shadow-xs'
          }`}
        >
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 rounded-xl bg-amber-100 dark:bg-amber-950/80 text-amber-700 dark:text-amber-400 flex items-center justify-center text-xl font-black flex-shrink-0">
              ⏳
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] text-slate-500">{req.timeAgo}</span>
              </div>
          ...
          {onSimulateNeighborArrival && (
            <button
              ...
              className="bg-amber-500 hover:bg-amber-600 text-stone-950 font-bold px-3 py-1.5 rounded-xl text-xs flex items-center gap-1 shadow-xs cursor-pointer transition flex-shrink-0 self-end sm:self-auto"
